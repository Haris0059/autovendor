...
11. ✅ **Category automapping — OLX keyword suggestions** (unplanned addition, requested during manual testing) — the mapping dialog now suggests OLX categories via OLX's own public suggester `GET /categories/suggest?keyword=` (documented in `olx-api-docs/categories.md`; semantic — ranked by where real listings with that keyword live, so "dijamantski kroneri" → "Kroneri/Glodala" even with zero name overlap).
   - **Pinned response shape**: `{data: [{id, count, name, parent_categories: [immediate parent, …, root]}]}` — proxied as `GET /olx/categories/suggest?keyword=` → `[{id, name, count, path}]` with `path` reversed to root-first ("Biznis i Industrija > Mašine i alati"). Public upstream, but behind our JWT like all catalog routes; **deliberately uncached** (unbounded keyword space vs the 24h `olx-categories` TTL; cheap dialog-triggered GET). `GET /categories/find?name=` also exists upstream — unused for now.
   - Dialog: keyword auto-seeds from the chosen Woo category name (editable), debounced 400 ms (`useDebouncedValue` — first debounce util in the repo); one click on a suggestion selects the OLX category directly (`suggestedOlxCat` slots into the `effectiveOlxCat` precedence: cascade > suggestion > editing fallback), the 4-level cascade stays as manual fallback; attribute-defaults flow unchanged. `MissingServletRequestParameterException` now maps to 400 (was falling into the 500 catch-all).
   - Live-verified end-to-end through the UI: Woo "Dijamantski Kroneri" → suggestions → "Za obradu keramike i stakla" → required defaults blocked save until set → saved ("2 atr." badge) → edit reopens with keyword seeded → deleted (cleanup). 179 tests passing.
   - Future: product-title-based suggestions, LLM bulk mapping ("Mapiraj sve").
